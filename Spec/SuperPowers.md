...
- **Directive:** Before implementation, the agent MUST ask clairfying questions to explore edge cases, dependency conflicts, and user intent.
...
- **Atomic Commits/Changes:** Prefer making small, functional updates rather than massive, sweeping refactors. AI agent does not commit changes. Human developer commits scm/Git changes, AI agent should suggest scm commits at appropiate times (after a major bug is fixed, major feature delivered, project phase completed ... ).
...
- **API Consideration:** If project is to be used as a library, consider clean, friendly APIs when designing solutions. Ask human developer if 'API consideration' is to be considered or ignored as a clairfying question.
